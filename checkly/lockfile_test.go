...
func TestExtractPackageVersionFromBunLock(t *testing.T) {
	t.Parallel()
  ...
  "packages": {
    "@playwright/test": ["@playwright/test@1.48.0", "", { "dependencies": { "playwright": "1.48.0" } }, "sha512-xxx"],
    "playwright": ["playwright@1.48.0", "", {}, "sha512-yyy"],
  },
}
`,
			want: "1.48.0",
		},
		{
			...
			packageName: "@playwright/test",
			input: `{
  // bun.lock generated by bun
  "lockfileVersion": 1,
  /* block comment */
  "packages": {
    "@playwright/test": ["@playwright/test@1.50.1", "", {}, "sha512-xxx"],
  },
}
`,
			want: "1.50.1",
  ...
  "lockfileVersion": 1,
  "packages": {
    "packages/e2e/@playwright/test": ["@playwright/test@1.45.0", "", {}, "sha512-xxx"]
  }
}
`,
			want: "1.45.0",
		...
		{
			name:        "npm alias prefix",
			packageName: "@playwright/test",
			input: `{
  "lockfileVersion": 1,
  "packages": {
    "@playwright/test": ["@playwright/test@npm:1.48.0", "", {}, "sha512-xxx"]
  }
}
`,
			want: "1.48.0",
		...
		{
			name:        "skips workspace protocol entries",
			packageName: "@playwright/test",
			input: `{
  "lockfileVersion": 1,
  "packages": {
    "@playwright/test": ["@playwright/test@workspace:packages/playwright-stub"]
  }
}
`,
			want: "",
		...
		{
			name:        "skips git protocol entries",
			packageName: "@playwright/test",
			input: `{
  "lockfileVersion": 1,
  "packages": {
    "@playwright/test": ["@playwright/test@git+https://github.com/microsoft/playwright.git#main"]
  }
}
`,
			want: "",
		...
		{
			name:        "preserves // inside string values",
			packageName: "@playwright/test",
			input: `{
  "lockfileVersion": 1,
  "packages": {
    "@playwright/test": ["@playwright/test@1.48.0", "https://registry.npmjs.org/", {}, "sha512-xxx"],
  },
}
`,
			want: "1.48.0",
  ...
  "lockfileVersion": 1,
  "packages": {
    "typescript": ["typescript@5.3.2", "", {}, "sha512-xxx"]
  }
}
`,
			want: "5.3.2",
...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			got, err := extractPackageVersionFromBunLock(strings.NewReader(tt.input), tt.packageName)
			if (err != nil) != tt.wantErr {
				t.Fatalf("error = %v, wantErr %v", err, tt.wantErr)
			}
			if got != tt.want {
