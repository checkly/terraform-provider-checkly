...
- `frequency_offset` (Number) Only relevant when `type` is `API`. When `frequency` is `0` (high frequency), `frequency_offset` alone controls how often the monitor should run. Defined in seconds. The allowed values are `10` (10 seconds), `20` (20 seconds) and `30` (30 seconds).
