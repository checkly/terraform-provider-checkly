...
page_title: "checkly_grpc_monitor Resource - terraform-provider-checkly"
subcategory: ""
description: |-
...
- `request` (Block List, Min: 1, Max: 1) The parameters for the gRPC connection. (see [below for nested schema](#nestedblock--request))
...
- `degraded_response_time` (Number) The response time in milliseconds starting from which a monitor should be considered degraded. Possible values are between 0 and 30000. (Default `4000`).
...
- `max_response_time` (Number) The response time in milliseconds starting from which a monitor should be considered failing. Possible values are between 0 and 30000. (Default `5000`).
...
- `host` (String) The host to connect to. Do not include a scheme or a port in this value.
- `port` (Number) The port number to connect to. Possible values are between 1 and 65535.
...
- `metadata` (Block Set) gRPC metadata (request headers) sent with the call. (see [below for nested schema](#nestedblock--request--metadata))
- `method` (String) The fully-qualified gRPC method to invoke in `BEHAVIOR` mode (e.g. `package.Service/Method`). Required in `BEHAVIOR` mode; forbidden in `HEALTH` mode.
- `proto_content` (String) The inline `.proto` file source used when `service_definition = "PROTO_FILE"` in `BEHAVIOR` mode.
...
### Nested Schema for `request.metadata`

Required:

- `key` (String) The gRPC metadata (header) key.

Optional:

- `value` (String) The gRPC metadata (header) value.
